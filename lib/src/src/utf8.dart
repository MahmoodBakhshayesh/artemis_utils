...
List<int> codepointsToUtf8(List<int> codepoints, [int offset = 0, int length=0]) {
  ...
  var encoded = List<int>.generate(encodedLength, (index) => 0);
  var insertAt = 0;
  for (var value in source) {
    if (value < 0 || value > UNICODE_VALID_RANGE_MAX) {
...
class Utf8Decoder implements Iterator<int> {
  ...
  List<int> decodeRest() {
    ...
    var codepoints = List<int>.generate(utf8EncodedBytesIterator.remaining, (index) => 0);
    var i = 0;
    while (moveNext()) {
    ...
    if (i == codepoints.length) {
      ...
      var truncCodepoints = List<int>.generate(i, (index) => 0);
